...
if range(A)==0    % if all elements are the same,  it is one big run
    if A(1)==varargin{1}   % if it's the number you are looking for
        runs{1,1}=A(1);
        runs{1,2}=[A(1) A(end)];
    else
        runs={};
    ...
    for numCount = 1:length(num),
        ...
        if ~isempty(indexVect)  % adjusted by MJR 8.10.2016 to account for case where not all requested 
