function [S,state,avg_wake_episodes_vs_time,avg_SWS_episode_duration_vs_time,avg_REMS_episode_duration_vs_time]=two_process_model_with_markov_chain(total_time,taui,taud,shift,makeplots)
...
ahat_S_R_base=0.007;
ahat_R_S_base=0.0035;
% Now make the ahat values depend on S-C, a measure of sleepiness from Achermann 2004.  Wake depends on 1-S+C, a measure of alertness from Achermann 2004  
% *****  ahat_R_S = alpha*(S-C)
% *****  ahat_R_W = beta*(S-C)
% *****  ahat_S_W = gamma*(S-C)
% *****  ahat_S_R = delta*(S-C)
...
% *****  ahat_W_R = mu*(1-S+C)
alpha = 0.003/0.3;  % I'm using 0.3 because that is the average of the S-C curve with no sleep dep
beta  = 1.2e-4/0.3;
gamma = 0.006/0.3;
delta = 0.007/0.3;
sigma = 0.004/0.65;
...
for run = 1:num_simulations
	...
	while i<length(t)
		...
		ahat_R_S(i) = alpha*(S(i,run)-C(i));
		ahat_R_W(i) = beta *(S(i,run)-C(i));
		ahat_S_W(i) = gamma*(S(i,run)-C(i));
		...
		if state(i,run)=='W'
			...
			if i+Wshortest_epochs > length(t) Wshortest_epochs = length(t)-i; end % truncate if the wait time puts you past the end of t
			if W_RW < W_SW
				next_state = 'R';
			else
				next_state = 'S';
			...
			for j=1:Wshortest_epochs				 % update the homeostat
				S(i+j,run)=1-(1-S(i+j-1,run))*exp(-dt/taui);
			end
		...
		elseif state(i,run)=='S'
			...
			for j=1:Wshortest_epochs
				S(i+j,run) = S(i+j-1,run)*exp(-dt/taud);
			end
		...
		if strcmp(next_state,'W') | strcmp(next_state,'R')
			S(i+Wshortest_epochs+1,run) = 1-(1-S(i+Wshortest_epochs,run))*exp(-dt/taui);
		else
			S(i+Wshortest_epochs+1,run) = S(i+Wshortest_epochs,run)*exp(-dt/taud); 
		end

		% Finally, override the Markov Chain sleep state if sleep deprivation is turned on
		if i+Wshortest_epochs+1 < length(sleep_dep)
			if sum(find(sleep_dep(i+1:i+Wshortest_epochs+1)))>0
				...
				for j=sleep_dep_locs_this_cycle
					state(i+1+j-1,run) = 'W';
					S(i+1+j-1,run) = 1-(1-S(i+1+j-2,run))*exp(-dt/taui);
				end
			end
		end
...
% ---- Compute average episode length -----
 [avg_wake_episodes_vs_time,avg_SWS_episode_duration_vs_time,avg_REMS_episode_duration_vs_time] = group_and_plot_Wepisodes_SWS_REMS_durationW1(shift,state,num_simulations);
